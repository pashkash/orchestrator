...
# type: METHOD
# use_case: Resolves the next phase from the current status and the transition table in the flow manifest.
# feature:
#   - top-level graph in V1 is controlled only by phase statuses
#   - orchestrator/config/flow.yaml
# pre:
#   - state.current_status is set
#   - transition table contains an entry for phase_id + current_status
# post:
#   - returns target phase id or `end`
# invariant:
