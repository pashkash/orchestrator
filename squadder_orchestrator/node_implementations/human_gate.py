...
# type: METHOD
# use_case: Останавливает граф на human gate и переводит решение пользователя обратно в PipelineState.
# feature:
#   - V1 orchestration допускает interrupt/resume без потери mutable plan
#   - Task card 2026-03-24_1800__multi-agent-system-design, D3-D5
# pre:
#   - state.trace_id или context trace_id доступны
# post:
#   - возвращает partial PipelineState с human_decisions и PASS/BLOCKED статусом
# invariant:
#   - ранее накопленные human_decisions не теряются
# modifies (internal):
#   - file.task_history
# emits (external):
