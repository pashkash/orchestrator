...
from squadder_orchestrator.node_implementations.status_aggregation import merge_structured_outputs
...
# type: METHOD
# use_case: Выполняет validate-фазу поверх merged structured outputs и формирует final_result.
# feature:
#   - Cross-cutting validation в V1 работает после execute и может вернуть task в replan/human gate
#   - Task card 2026-03-24_1800__multi-agent-system-design, D3-D4
# pre:
#   - phase_config.pipeline определён
# post:
#   - возвращает partial PipelineState с merged_summary и current_status validate-фазы
# invariant:
#   - structured_outputs из state не мутируются
# modifies (internal):
#   - file.task_history
# emits (external):
