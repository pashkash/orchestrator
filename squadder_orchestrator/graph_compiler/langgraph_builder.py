...
from squadder_orchestrator.agent_drivers import MockDriver, OpenHandsDriver
from squadder_orchestrator.agent_drivers.base_driver import BaseDriver
from squadder_orchestrator.graph_compiler.edge_evaluators import collect_phase_targets, resolve_next_phase
from squadder_orchestrator.graph_compiler.state_schema import DriverMode, PipelineState
from squadder_orchestrator.graph_compiler.yaml_manifest_parser import FlowManifest, RuntimeConfig
...
from squadder_orchestrator.integrations.phase_config_loader import get_flow_manifest, get_runtime_config
from squadder_orchestrator.node_implementations.human_gate import run_human_gate
from squadder_orchestrator.node_implementations.phases.collect_phase import run_collect_phase
from squadder_orchestrator.node_implementations.phases.execute_phase import run_execute_phase
from squadder_orchestrator.node_implementations.phases.plan_phase import run_plan_phase
from squadder_orchestrator.node_implementations.phases.validate_phase import run_validate_phase
...
# type: METHOD
# use_case: Строит runtime driver для graph compilation из выбранного режима и runtime-config.
# feature:
#   - Один и тот же phase graph может работать поверх mock или OpenHands runtime
#   - Task card 2026-03-24_1800__multi-agent-system-design, D4-D5
# pre:
#   - driver_mode поддерживается
#   - Для openhands заданы base_url_env и llm_api_key_env
# post:
#   - возвращает готовый BaseDriver
# invariant:
#   - graph contract не меняется из-за driver implementation
# modifies (internal):
#   -
# emits (external):
#   -
...
# type: METHOD
# use_case: Компилирует V1 phase graph из runtime manifests и выбранного driver mode.
# feature:
#   - Graph topology берётся из `orchestrator/config/flow.yaml`
#   - phase behaviour берётся из `orchestrator/config/phases_and_roles.yaml`
# pre:
#   - manifest-ы валидны и содержат collect/plan/execute/validate/human_gate
# post:
#   - возвращает compiled LangGraph runnable для V1 pipeline
# invariant:
#   - Python builder не хардкодит бизнес-ветвления кроме phase wrapper registry
# modifies (internal):
...
def compile_graph(
    *,
    ...
    selected_driver_mode = DriverMode(
        str(driver_mode or os.getenv("SQUADDER_ORCHESTRATOR_DRIVER_MODE", DriverMode.MOCK))
    )
