...
# type: METHOD
# use_case: Загружает top-level phase graph manifest из `orchestrator/config/flow.yaml`.
# feature:
#   - V1 graph состоит только из collect/plan/execute/validate/human_gate фаз
#   - Task card 2026-03-24_1800__multi-agent-system-design, D0-D3
# pre:
#   - path существует
# post:
#   - возвращает типизированный FlowManifest
# invariant:
#   - входной YAML не мутируется
# modifies (internal):
...
# type: METHOD
# use_case: Загружает runtime phase config из `orchestrator/config/phases_and_roles.yaml`.
# feature:
#   - runtime config описывает universal TaskUnit и OpenHands integration path
#   - Task card 2026-03-24_1800__multi-agent-system-design, D0-D7
# pre:
#   - path существует
# post:
#   - возвращает RuntimeConfig с per-phase pipeline settings
# invariant:
#   - входной YAML не мутируется
# modifies (internal):
