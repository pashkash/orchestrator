...
class OpenHandsHttpApi:
    ...
    # type: METHOD
    # use_case: Создаёт новую OpenHands conversation через REST API.
    # feature:
    #   - Phase 3 runtime path должен запускать real OpenHands agent server
    #   - openhands/software-agent-sdk REST API
    # pre:
    #   - payload содержит agent, workspace и initial_message
    # post:
    #   - возвращает handle с conversation_id и initial state
    # invariant:
    #   - HTTP client остаётся открытым после вызова
    # modifies (internal):
    ...
    # type: METHOD
    # use_case: Poll-ит OpenHands conversation пока она не перейдёт в terminal status.
    # feature:
    #   - Phase 3 driver должен дождаться завершения remote run перед парсингом результата
    #   - openhands/software-agent-sdk REST API
    # pre:
    #   - conversation_id не пустой
    # post:
    #   - возвращает последний conversation state из OpenHands server
    # invariant:
    #   - client reuse сохраняется
    # modifies (internal):
    #   - external.openhands_server
    # emits (external):
    #   - external.openhands_server
    # errors:
    #   - TimeoutError: время ожидания превышено
    # depends:
    #   - get_conversation
